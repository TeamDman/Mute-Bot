...
commands.getChannel = identifier => {
  if (typeof identifier === 'string') {
    if (identifier.match(/\d+/g)) { identifier = identifier.match(/\d+/g); }
  }
  for (let guild of client.guilds.values()) {
...
commands.getUser = (guild, identifier) => {
  if (typeof identifier === 'string') {
    if ((identifier = identifier.replace(/\s+/g, '_').toLowerCase()).match(/\d+/g)) { identifier = identifier.match(/\d+/g); }
  }
  for (let member of guild.members.values()) {
    if (member.id == identifier || member.user.username.replace(/\s+/g, '_').toLowerCase() == identifier) { return member; }
  }
  return null;
};
...
commands.mute = async member => {
  ...
  for (let channel of member.guild.channels.values()) { channel.overwritePermissions(member, {SEND_MESSAGES: false}); }
};
...
commands.unmute = async member => {
  ...
  for (let channel of member.guild.channels.values()) { channel.permissionOverwrites.get(member.id).delete('pardoned'); }
};

commands.getClaims = user => {
  if (claims[user.id] === undefined) { return claims[user.id] = {}; } else { return claims[user.id]; }
};
  ...
  try {
    ...
    let handle = (reaction, user) => {
      ...
      if (user.id !== sourceMessage.author.id ||
                ...
                reaction.emoji.name !== emojistop) { return; }
  ...
  msg.createReactionCollector((react, user) => user.id !== client.user.id).on('collect', async (reaction, collector) => {
    switch (reaction.emoji.name) {
      case '\uD83D\uDC80':
        ...
        if (config.infraction_ban_notify_enable) { message.author.createDM().then(c => c.send(config.infraction_ban_notify_message).catch(e => console.error(e))).catch(e => console.error(e)); }
      ...
      case '✅':
        ...
        if (config.infraction_pardon_notify_enable) { message.author.createDM().then(c => c.send(config.infraction_pardon_notify_message).catch(e => console.error(e))).catch(e => console.error(e)); }
        commands.report(new discord.RichEmbed()
...
commands.onMessage = async message => {
  ...
  if (message.content.indexOf(config.prefix) !== 0) {
    if ((pattern = config.patterns.find(p => message.content.match(new RegExp(p))))) {
      ...
      config.link_filter_settings.enabled &&
      !message.member.roles.has(config.link_filter_settings.role_required) &&
      message.content.indexOf('http://') !== -1
    ) {
  ...
  if (!commands.hasPerms(message.member)) { return message.channel.send('You do not have permissions to use this command.'); }
...
addCommand({name: 'set'}, async (message, args) => {
  switch (args.shift().toLowerCase()) {
    case 'channel':
      ...
      if (channel === null) { return message.channel.send('Unable to find the channel specified.'); }
    ...
    case 'role':
      ...
      if (role === null) { return message.channel.send('Unable to find the role specified.'); }
...
addCommand({name: 'patterns'}, async (message, args) => {
  switch (args.shift().toLowerCase()) {
    case 'add':
      if (config.patterns.includes(args.join(' '))) { return message.channel.send(new discord.RichEmbed().setColor('ORANGE').setDescription(`That pattern is already in the list.`)); }
    ...
    case 'remove':
      ...
      if (!config.patterns[r]) { return message.channel.send(new discord.RichEmbed().setColor('ORANGE').setDescription(`The given index is invalid.`)); }
    ...
    case 'list':
      ...
      for (i = 0, j = config.patterns.length; i < j; i += chunkSize) {
        ...
        for (let role in chunk) { embed.description += `**${role}**: \`${chunk[role]}\`\n`; }
        pages.push(embed);
      }
